...
export function dynamicStyle(element, parameters = default_parameters) {
    // Cache the existing styles on the element in a store
    const store = writable(parameters);
    const split_static_rules = derived(store, ($store, set) => {
        if ($store.styles) {
            const rules = splitStyleRules($store.styles);
            set(rules);
        }
    });
    const split_hover_rules = derived(store, (store_value, set) => {
        if (store_value.hover_styles) {
            const rules = splitStyleRules(store_value.hover_styles);
            set(rules);
        }
    });
    const split_active_rules = derived(store, (store_value, set) => {
        if (store_value.active_styles) {
            const rules = splitStyleRules(store_value.active_styles);
            set(rules);
        }
    });
    const split_focus_rules = derived(store, ($store, set) => {
...
function splitStyleRules(styles) {
    try {
        return styles.split(';');
    }
    catch (error) {
