...
export function tooltip(node, parameters = default_parameters) {
    try {
        // Find max transition/animation time on ancestor nodes
        const ancestor_nodes = getAncestors(node);
        ...
        const dialog = ancestor_nodes?.find((ele) => ele?.tagName === 'DIALOG');
        ...
        const tooltip_parameters = writable({
            ...default_parameters,
            ...parameters
        });
        let title_attribute;
        ...
        const { visible, in_delay, out_delay, visibility_delay, ...all_other_new_tooltip_parameters } = new_tooltip_parameters;
        ...
        const { visibility_delay: v_delay, delay: total_delay, in_delay: i_delay } = get(tooltip_parameters);
        const delays = [];
        if (v_delay)
            delays.push(v_delay);
        if (total_delay)
        ...
        async function changeVisiblityAfterDelay({ visibility, delay }) {
            setTimeout(async () => {
        ...
        function getParametersForNewTooltip() {
            const { 
            // eslint-disable-next-line @typescript-eslint/no-unused-vars
            horizontal_offset, 
            // eslint-disable-next-line @typescript-eslint/no-unused-vars
            ...
            in_delay, 
            // eslint-disable-next-line @typescript-eslint/no-unused-vars
            visibility_delay: positioning_delay, ...passing_parameters } = get(tooltip_parameters);
            return passing_parameters;
        }
        async function makeInvisibleTooltip(parameters) {
        ...
        async function initializeTooltipPosition(allow_offscreen = false) {
            ...
            await positionTooltip(node, invisible_tooltip, new_tooltip_parameters.position ?? 'top', new_tooltip_parameters.allow_dynamic_position, horizontal_offset, vertical_offset, allow_offscreen);
        ...
        async function changeOffsets(old_parameters, new_parameters) {
            ...
            if ((old_position === 'left' && new_position === 'right') ||
                (old_position === 'right' && new_position === 'left')) {
                tooltip_parameters.update((current_parameters) => {
                    return {
                        ...current_parameters,
                        horizontal_offset: getHorizontalOffset(new_position, current_parameters.horizontal_offset, true)
                    };
                });
            }
            else if ((old_position === 'top' && new_position === 'bottom') ||
                ...
                tooltip_parameters.update((current_parameters) => {
                    return {
                        ...current_parameters,
                        horizontal_offset: getHorizontalOffset(new_position, current_parameters.horizontal_offset),
        ...
        async function positionTooltip(anchor_element, tooltipComponent, new_position, allow_dynamic_position, horizontal_offset, vertical_offset, allow_offscreen = false) {
            let x;
            let y;
            ...
            const parent_styles = anchor_element?.parentElement
            ...
            const { clientWidth: tip_width = 0, offsetWidth: tip_offset_width = 0, clientHeight: tip_height = 0, offsetHeight: tip_offset_height = 0, marginLeft: tip_margin_left = 0, marginTop: tip_margin_top = 0, marginRight: tip_margin_right, paddingLeft: tip_padding_left, paddingRight: tip_padding_right
        ...
        return {
            ...
            async goToNextNode(next_index, next_delay = 0) {
...
function getHorizontalOffset(position, designated_offset, should_invert) {
...
function getVerticalOffset(position, designated_offset, should_invert) {
    if (designated_offset !== undefined) {
        return should_invert ? -1 * designated_offset : designated_offset;
    }
    if (position === 'bottom') {
...
async function getParentPaddings(parent_styles) {
    const { paddingLeft, paddingRight, paddingTop, paddingBottom } = parent_styles;
    const parent_padding_left = parseInt(paddingLeft.replace('px', ''));
    const parent_padding_right = parseInt(paddingRight.replace('px', ''));
    const parent_padding_top = parseInt(paddingTop.replace('px', ''));
    const parent_padding_bottom = parseInt(paddingBottom.replace('px', ''));
    return { parent_padding_top, parent_padding_right, parent_padding_bottom, parent_padding_left };
}
async function positionX(input) {
    const { position, left, width, horizontal_middle, tip_offset_width, tip_margin_left, horizontal_offset = 0 } = input;
    if (position === 'left') {
...
export async function positionXAbsolute(input) {
    const { position, horizontal_middle, tip_offset_width, tip_margin_left = 0, tip_margin_right = 0, tip_padding_left = 0, horizontal_offset = 0, parent_styles } = input;
...
async function repositionX(input) {
    ...
    if (x < 0)
        x = 0 + min_margin;
    if (x + tip_offset_width > screen_width)
        x = screen_width - tip_offset_width - min_margin;
    return x;
}
async function positionY(input) {
    const { position, top, bottom, vertical_middle, tip_offset_height, tip_margin_top, vertical_offset = 0 } = input;
...
async function positionYAbsolute(input) {
    const { position, top, bottom, vertical_middle, tip_offset_height, tip_margin_top, vertical_offset = 0, parent_styles } = input;
    const { parent_padding_left, parent_padding_right, parent_padding_top, parent_padding_bottom } = await getParentPaddings(parent_styles);
    if (position === 'left' || position === 'right') {
...
async function repositionY(input) {
    ...
    if (y < 0)
        y = 0 + min_margin;
    if (y + tip_offset_height > screen_height)
        y = screen_height - tip_offset_height - min_margin;
    return y;
}
