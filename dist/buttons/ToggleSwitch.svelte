...
export let bgColorHue = null; // = 246; // 207;
export let bgColorSaturation = null; // = 46; // 90;
export let bgColorLuminosity = null; // = 37; // 54;
...
export let inactiveBgColorDarken = `hsl(${inactiveBgColorHue}, ${inactiveBgColorSaturation}%, ${inactiveBgColorLuminosity - darkenHoverPercentage}%)`;
export let toggleBgColorActive = bgColorHue && bgColorSaturation && bgColorLuminosity
    ? `hsl(${bgColorHue},${bgColorSaturation}%,${bgColorLuminosity}%)`
    : null;
export let toggleBgColorActiveHovered = bgColorHue && bgColorSaturation && bgColorLuminosity
...
.switch {
  ...
  border: none;
  border-radius: 1em;
  height: var(--toggle-height, calc(var(--scale, 1rem) * 1.7));
}
...
.slider:before {
  ...
  height: var(--slider-height, var(--scale, 1rem));
  width: var(--slider-width, var(--scale, 1rem));
  left: 10%;
  top: 20%;
...
/* Rounded sliders */
.slider.round {
  border-radius: var(--toggle-height, calc(var(--scale, 1rem) * 1.7));
  padding: 1px;
}
