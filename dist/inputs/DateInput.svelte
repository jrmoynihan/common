...
/** The date to display */
export let date;
export let min = new Date(-8640000000000000);
export let max = new Date(8640000000000000);
export let name = '';
...
export let date_input = null;
export let label_element = null;
...
const convertDateToString = (date) => {
    ...
    const newString = `${date.getFullYear()}-${date.getMonth() < 10 ? '0' + (date.getMonth() + 1) : date.getMonth() + 1}-${date.getDate() < 10 ? '0' + date.getDate() : date.getDate()}`;
    return newString;
};
const output = (stringDate) => {
    // NOTE: remove the +1 offset to return the correct month
    const newDate = new Date(Number(stringDate.split('-')[0]), Number(stringDate.split('-')[1]) - 1, Number(stringDate.split('-')[2]));
    return newDate;
};
