...
export function realShadow(input = defaultShadowParameters) {
    const internal = { ...defaultShadowParameters, ...input };
    const { resolution, oomph, crispness, hsl, elevation, vertical_distance, horizontal_distance } = internal;
    ...
    const shadows = Array(shadow_count)
        ...
        return `${x.toPrecision(2)}px ${y.toPrecision(2)}px ${shadow_blur}px ${current_shadow > 0 ? `${spread < 0 ? spread.toPrecision(2) : ''}px` : ''} hsla(${shadow_color} / ${alpha})`;
    });
