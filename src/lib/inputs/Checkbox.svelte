...
<style lang="scss">
    label {
        ...
        --checkbox-default-transition-function: 
            var(--checkbox-transition-speed, var(--checkbox-default-transition-speed))
            var(--checkbox-transition-timing-function, var(--checkbox-default-transition-timing-function));
        --checkbox-default-checked-transition: 
        ...
        --checkbox-default-label-hover-background-color: oklch(from var(--background) calc(l + 0.2) c h);
        ...
        background-color: var(--checkbox-label-background-color, var(--checkbox-default-label-background-color));
        user-select: var(--checkbox-user-select, var(--checkbox-default-user-select));
		&:hover{
			background-color: var(--checkbox-label-hover-background-color, var(--checkbox-default-label-hover-background-color));
		}
        &:has(> input[type='checkbox']:disabled) {
            opacity: var(--checkbox-label-disabled-opacity, var(--checkbox-default-label-disabled-opacity));
            cursor: not-allowed;
        }
        & > input[type='checkbox'] {
