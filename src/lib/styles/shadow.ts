...
export function realShadow(
	input: RealShadowParameters = defaultShadowParameters,
) {
	...
	const max_shadows =
		elevation === "high" ? 10 : elevation === "medium" ? 5 : 3;
	const shadow_count =
		Math.ceil(resolution * (max_shadows - min_shadows)) + min_shadows;

	const shadow_blur =
		(horizontal_offset_factor + vertical_offset_factor) * crispness * oomph;
	...
	const shadows = Array(shadow_count)
		...
		.map((_, current_shadow) => {
			...
			const alpha =
				base_alpha - (base_alpha / shadow_count) * (current_shadow - 1);
