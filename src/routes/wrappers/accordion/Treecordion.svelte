<script lang="ts">
	...
	const node_1_3_items = [
		{
			url: new URL('Descendant_selectors', base),
			text: 'Descendant'
		},
		{
			url: new URL('Child_selectors', base),
			text: 'Child'
		},
		{
			url: new URL('Adjacent_sibling_selectors', base),
			text: 'Adjacent'
		},
		{
			url: new URL('General_sibling_selectors', base),
			text: 'General'
		}
	];
	const node_1_4_items = [
		{
			url: new URL('Pseudo-classes', base),
			text: 'Pseudo-class'
		},
		{
			url: new URL('Pseudo-elements', base),
			text: 'Pseudo-element'
		}
	];
</script>

{#snippet summary(text: string)}
...
{#snippet details(children: Snippet<[]>)}
	<details class="tree-item-details">
		{@render children()}
	</details>
{/snippet}

{#snippet tree_item_label(text: string, url: URL)}
		...
		>
			<TreeList node_to="--node-1">
						...
						<TreeList node_to="--node-1-1">
							{#each node_1_1_items as item, i}
								<TreeItem is_node="--node-1-1-{i + 1}">
									{@render tree_item_label(item.text, item.url)}
								</TreeItem>
							{/each}
						</TreeList>
					</AccordionDetails>
				...
				<TreeItem is_node="--node-1-2">
					{@render tree_item_label(
						'Selector list',
						new URL('https://developer.mozilla.org/en-US/docs/Web/CSS/Attribute_selectors')
					)}
				</TreeItem>
						...
						<ul class="tree" style="--to: --node-1-1">
							...
							<li class="tree-item" style="--is: --node-1-1-2">
								<p class="tree-item-label">
									<a
										target="_blank"
										href="https://developer.mozilla.org/en-US/docs/Web/CSS/Type_selectors">Type</a
									>
								</p>
							</li>
							<li class="tree-item" style="--is: --node-1-1-3">
								<p class="tree-item-label">
									<a
										target="_blank"
										href="https://developer.mozilla.org/en-US/docs/Web/CSS/Class_selectors">Class</a
									>
								</p>
							</li>
							<li class="tree-item" style="--is: --node-1-1-4">
								<p class="tree-item-label">
									<a
										target="_blank"
										href="https://developer.mozilla.org/en-US/docs/Web/CSS/ID_selectors">ID</a
									>
								</p>
							</li>
